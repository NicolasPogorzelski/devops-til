...
| [SQLite on CIFS Locking](storage/sqlite-on-cifs-locking.md) | Why `database is locked` over CIFS (byte-range locks not honored), CIFS-vs-local control test, `nobrl` vs the local-copy + atomic-swap workaround |
