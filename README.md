...
| [Dotfiles Management](operations/dotfiles-management.md) | Template + render pattern, `--dry-run` flag, `pipx ensurepath` PATH fix, validate.sh, bootstrap/install split, **template rot** (diff template vs live before `install.sh`), `uv tool install` as the `pipx` shape on immutable Fedora |
