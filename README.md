...
| [umask Is Shell State](linux/umask-shell-state.md) | `umask 077 && cmd` changes the *shell*, not the command: a later `git pull` wrote a bind-mounted Caddyfile as 600 and the proxy crash-looped; create secret files with `install -m 600 /dev/null` instead, scope state-changing commands in a subshell, repair modes with `git ls-files -s` |
...
| Topic | Summary |
|---|---|
| [Reverse Proxy: X-Forwarded-* Headers](networking/reverse-proxy-forwarded-headers.md) | Why a TLS-terminating proxy loses scheme and client IP for the backend, what `X-Forwarded-Proto`/`-For` restore, and why nginx `real_ip`, Tomcat `RemoteIpValve` and Rails only honour them from a trust list; the proxy must strip client-sent headers (verified with a forged header); residual risk of trusting a whole Docker pool |
...
| Topic | Summary |
|---|---|
| [Application-Consistent Backup & Restore](docker/application-consistent-backup-restore.md) | State comes in shapes (files, SQLite, PostgreSQL, bundled products) and each needs its own method; stop the app, dump the DB, leave GitLab running; DB container first, then `pg_restore`, then the app; the product's backup tool defines *its* state (GitLab omits secrets **and SSH host keys**); `tar --numeric-owner` under userns-remap; a set is a secret (age before it leaves the host); `compose start` re-runs one-shot deps; `exit` inside a helper is not caught by `\|\|`; verification = machine list + functional list; measured RTO 21 min |
| [Container Hardening Ladder](docker/root-without-capabilities.md) | Non-root vs root-without-capabilities vs root: what `cap_drop: ALL` leaves a UID-0 process (owner bits work, no chown/setuid/bind<1024), why Tomcat runs that way and Omnibus cannot, why entrypoints that edit the image block non-root, and how `userns-remap` complements the in-container ladder; inspect/`CapEff` checks |
| [JVM Heap vs. Container Memory Limit](docker/jvm-heap-vs-container-memory-limit.md) | `-Xmx` is enforced by the JVM (`OutOfMemoryError`), `mem_limit` by the kernel (exit 137, no log line); a JVM needs heap + ~0.5-1 GiB for metaspace, thread stacks, code cache and native buffers, so the cgroup limit must exceed `-Xmx` by that margin; same pattern for Puma workers and PostgreSQL buffers; `docker stats`/`ExitCode` checks |
...
| [Claude Code Hooks](operations/claude-code-hooks.md) | Hook events, stdin JSON, `additionalContext` vs `systemMessage`, `if` conditional field, SessionStart context injection, dual-Stop pattern, defense-in-depth with branch protection, hook fatigue, **symlinked `/home` -> `/var/home` defeats a `pwd`-vs-`rev-parse` path comparison** (store physical paths, resolve both sides with `rev-parse`), test hooks with a hand-written stdin payload, **`additionalContext` on Stop means "continue"** (measured loop, `systemMessage` is the reminder form), hot-reload is a per-version measurement, `if:` runs anyway on `$VAR`/`$()`, **a push-refusal regex that never matched a plain push** (test patterns under `/usr/bin/grep` - the tool shell's `grep` is ugrep), timeout is a bypass width |
...
| [Dotfiles Management](operations/dotfiles-management.md) | Template + render pattern, `--dry-run` flag, `pipx ensurepath` PATH fix, validate.sh, bootstrap/install split, **template rot** (diff template vs live before `install.sh`), `uv tool install` as the `pipx` shape on immutable Fedora, **a drifted `~/.gitconfig` changes who you are** (seven public commits under a second identity; template is the truth, credential helpers are not template material), **commit signing is per-workstation** (`gpgsig` header beats `%G?`, read the signing key out of a signed commit) |
